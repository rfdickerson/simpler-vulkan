...
void createTreePipeline(Device& device, Swapchain& swapchain, TreePipeline& pipeline, VkFormat depthFormat) {
    ...
    // Binding 0: per-vertex data (box geometry)
    bindingDescriptions[0].binding = 0;
    bindingDescriptions[0].stride = sizeof(BoxVertex);
    bindingDescriptions[0].inputRate = VK_VERTEX_INPUT_RATE_VERTEX;
    
    // Binding 1: per-instance data
    bindingDescriptions[1].binding = 1;
    bindingDescriptions[1].stride = sizeof(TreeInstance);
    ...
    attributeDescriptions[0].offset = offsetof(BoxVertex, position);
    
    // Location 1: instance position
    attributeDescriptions[1].binding = 1;
    attributeDescriptions[1].location = 1;
    attributeDescriptions[1].format = VK_FORMAT_R32G32B32_SFLOAT;
    attributeDescriptions[1].offset = offsetof(TreeInstance, position);
    
    // Location 2: instance rotation
    attributeDescriptions[2].binding = 1;
    attributeDescriptions[2].location = 2;
    attributeDescriptions[2].format = VK_FORMAT_R32_SFLOAT;
    attributeDescriptions[2].offset = offsetof(TreeInstance, rotation);
    ...
    if (vkCreatePipelineLayout(device.device, &pipelineLayoutInfo, nullptr, &pipeline.pipelineLayout) != VK_SUCCESS) {
        throw std::runtime_error("Failed to create tree pipeline layout!");
    }
